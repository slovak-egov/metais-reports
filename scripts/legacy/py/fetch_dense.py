...
def write_global_dict(global_values: List[Any]) -> Dict[str, int]:
    ...
    meta = {
        ...
        "offsetByteSize": ROW_OFFSET_BYTES,
        "encoding": "utf-8",
        "format": "json",
        "endianness": "LE",
    }
...
def pack_node_type_streaming(
    ...
    """
    Dense streaming pack:

      nodes/TYPE.rows.bin          (variable-length rows)
      nodes/TYPE.rows.offsets.bin  (uint64 offsets, length N+1)
      nodes/TYPE.uuids.bin

    Each row:
        k: uint16
        (attrIndex:uint16, dictIndex:int32) * k
    """

    print(f"[nodes] Streaming *dense* pack for type {type_name}")
    ...
    offsets: List[int] = [0]  # offsets[r] = byte offset of row r in rows.bin
    current_off = 0

    pack_i32 = INT32_LE.pack
    pack_u16 = U16_LE.pack
    pack_u64 = U64_LE.pack

    with rows_path.open("wb") as frows, uuids_path.open("wb") as fuuid:
        for page in iter_node_pages(type_name):
            for rec in page:
                ...
                if u_bytes in seen_uuids_for_type:
                    ...
                    raise RuntimeError(
                        f"[nodes] {type_name}: duplicate UUID during streaming "
                ...
                # Collect (attrIndex, dictIndex) pairs for this row
                ...
                # Sort by column index so we can binary-search later
                if pairs:
                    pairs.sort(key=lambda p: p[0])

                # Write row: k + k*(attrIndex, dictIndex)
                k = len(pairs)
                frows.write(pack_u16(k))
                row_len = 2  # k

                for col_idx, dict_idx in pairs:
                    frows.write(pack_u16(col_idx))
                    frows.write(pack_i32(dict_idx))
                    row_len += 2 + 4
    ...
    # write offsets
    with offsets_path.open("wb") as foff:
        for off in offsets:
            foff.write(pack_u64(off))
    ...
    meta = {
        ...
        "dictIndexBytes": DICT_INDEX_BYTES,
        "endianness": "LE",
        "attributes": attributes_serialized,
        "sortedBy": "uuid_bytes",
        "typeName": type_name,
        "rowsFile": f"{type_name}.rows.bin",
        "rowOffsetsFile": f"{type_name}.rows.offsets.bin",
    }
    ...
    print(
        ...
        f"rows={rows_size / (1024*1024):.2f} MiB, "
...
def sort_node_type_by_uuid(type_name: str) -> None:
    """
    Post-processing pass for *dense* layout:

      - nodes/TYPE.uuids.bin is sorted by UUID bytes
      - nodes/TYPE.rows.bin and nodes/TYPE.rows.offsets.bin
        are reordered in lockstep
    ...
    if not meta_path.is_file() or not rows_path.is_file() \
       or not uuids_path.is_file() or not offsets_path.is_file():
    ...
    if meta.get("layout") != "dense":
        print(f"[sort-nodes] {type_name}: layout is not 'dense', skipping")
        return
    ...
    print(f"[sort-nodes] {type_name}: sorting {record_count} records by UUID (dense layout)")

    # 1) Read all UUIDs with their original index
    ...
    with offsets_path.open("rb") as f_off:
        for _ in range(record_count + 1):
            ...
            if len(raw) != ROW_OFFSET_BYTES:
                raise IOError(
                    f"[sort-nodes] {type_name}: unexpected EOF in rows.offsets.bin"
                )
            (off,) = U64_LE.unpack(raw)
            old_offsets.append(off)

    # 4) Rewrite uuids, rows, and offsets in sorted order
    tmp_uuids   = uuids_path.with_suffix(".uuids.tmp")
    tmp_rows    = rows_path.with_suffix(".rows.tmp")
    tmp_offsets = offsets_path.with_suffix(".offsets.tmp")

    with uuids_path.open("rb") as f_uuid_in, \
         rows_path.open("rb")  as f_rows_in, \
         tmp_uuids.open("wb")  as f_uuid_out, \
         tmp_rows.open("wb")   as f_rows_out, \
         tmp_offsets.open("wb") as f_off_out:

        # first offset is always 0
        current_off = 0
        f_off_out.write(U64_LE.pack(current_off))

        for new_idx, (uuid_bytes, old_idx) in enumerate(rows):
            # write UUID
            f_uuid_out.write(uuid_bytes)

            # copy row from old rows.bin
            start = old_offsets[old_idx]
            end   = old_offsets[old_idx + 1]
            length = end - start
            if length < 0:
                raise ValueError(
                    f"[sort-nodes] {type_name}: negative row length for old_idx={old_idx}"
                )

            f_rows_in.seek(start)
            data = f_rows_in.read(length)
            if len(data) != length:
                raise IOError(
                    f"[sort-nodes] {type_name}: unexpected EOF in rows.bin"
                )
            f_rows_out.write(data)
    ...
    print(f"[sort-nodes] {type_name}: done")
...
def pack_relations_from_uuid_pairs(
    ...
    for rel in sorted(reltypes):
        ...
        pair_record_bytes = 2 * UUID_BYTES
        if file_size % pair_record_bytes != 0:
            print(f"[WARN] {pair_path} size not divisible by {pair_record_bytes}; possible corruption", file=sys.stderr)
        record_count_raw = file_size // pair_record_bytes
...
def write_packed_manifest(
    ...
    if nodes_dir.is_dir():
        for meta_path in nodes_dir.glob("*.meta.json"):
            name = meta_path.name          # e.g. "KS.meta.json"
            tname = meta_path.stem         # e.g. "KS"
            ...
            if layout == "grid":
                ...
                entry["rowsFile"]       = meta.get("rowsFile", f"{tname}.rows.bin")
                entry["rowOffsetsFile"] = meta.get("rowOffsetsFile", f"{tname}.rows.offsets.bin")
...
def fetch_dense() -> None:
    ...
    for st in type_stats:
        print(
            ...
            f"rows={st['rowsSize'] / (1024*1024):.2f} MiB, "
            f"offsets={st['offsetsSize'] / (1024*1024):.2f} MiB, "
