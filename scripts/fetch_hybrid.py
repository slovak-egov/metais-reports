...
def build_global_uuid_index(
    ...
    bytes_per_code = TYPE_CODE_BYTES
    ...
    pack_code = get_uint_le_struct(bytes_per_code).pack
...
def pack_node_type_streaming(
    ...
    """
    Second streaming pass: for each record, write a fixed-size block of INT32_LE indices.
    """
    ...
    with bin_path.open("wb") as fbin, uuids_path.open("wb") as fuuid:
        for page in iter_node_pages(type_name):
            for rec in page:
                ...
                total_defined_attrs += defined_this_row
    ...
    meta = {
        ...
        "sortedBy": "uuid",                     # for binary search
        "typeName": type_name,
        "layout": "grid",
    }
    ...
    return {
        ...
        "metaSize": meta_size,
        "sumDefined": total_defined_attrs,
        "layout": "grid",
        "offsetsSize": 0,
    }
...
def estimate_dense_bytes(record_count: int, block_size: int, sum_defined: int) -> int:
    """
    Compute exact size of the dense layout for this type, given:

      N = record_count
      sum_defined = total number of non-missing cells over all rows

    Format:
      rows.bin:
        for each row: uint16 k + k * (uint16 attrIndex + int32 dictIndex)
      offsets.bin:
        (N+1) * uint64 offsets
    """
    N = record_count
    row_header_bytes = U16_LE.size
    per_value_bytes = ATTR_INDEX_BYTES + DICT_INDEX_BYTES
    rows_bytes = N * row_header_bytes + per_value_bytes * sum_defined
    offsets_bytes = (N + 1) * ROW_OFFSET_BYTES
    return rows_bytes + offsets_bytes

def convert_node_type_grid_to_dense(type_name: str, stats: Dict[str, Any]) -> Dict[str, Any]:
    """
    Convert nodes/<TYPE>.bin (grid layout) into:

      nodes/<TYPE>.rows.bin
      nodes/<TYPE>.rows.offsets.bin

    and update <TYPE>.meta.json to layout="dense" with denseEncoding.
    The per-row order is preserved (so if you sorted by UUID earlier,
    it remains UUID-sorted).
    """
    meta_path   = NODES_PACKED / f"{type_name}.meta.json"
    bin_path    = NODES_PACKED / f"{type_name}.bin"
    rows_path   = NODES_PACKED / f"{type_name}.rows.bin"
    offsets_path = NODES_PACKED / f"{type_name}.rows.offsets.bin"

    if not meta_path.is_file() or not bin_path.is_file():
        print(f"[dense] {type_name}: missing meta/bin, skipping conversion")
        return stats
    ...
    if record_count == 0 or block_size == 0:
        print(f"[dense] {type_name}: empty or degenerate, skipping")
        return stats

    if int_bytes != 4:
        raise ValueError(f"[dense] {type_name}: only intBytes=4 supported for now")

    block_bytes = block_size * int_bytes

    print(
        f"[dense] {type_name}: converting grid -> dense "
        f"(records={record_count}, blockSize={block_size})"
    )

    pack_u16 = U16_LE.pack

    with bin_path.open("rb") as f_in, \
         rows_path.open("wb") as f_rows, \
         offsets_path.open("wb") as f_off:

        offset = 0
        # first offset = 0
        f_off.write(U64_LE.pack(0))

        for row_idx in range(record_count):
            row_raw = f_in.read(block_bytes)
            if len(row_raw) != block_bytes:
                raise IOError(
                    f"[dense] {type_name}: unexpected EOF in .bin at row {row_idx}"
                )

            k = 0
            payload = bytearray()

            # scan the row, building dense representation
            for col_idx in range(block_size):
                offset_in_row = col_idx * int_bytes
                (val,) = INT32_LE.unpack_from(row_raw, offset_in_row)
                if val != MISSING_SENTINEL:
                    k += 1
                    payload += pack_u16(col_idx)          # attr index
                    payload += INT32_LE.pack(val)         # dict index

            # row header + payload
            f_rows.write(pack_u16(k))
            f_rows.write(payload)

            offset += row_header_bytes + len(payload)
            f_off.write(U64_LE.pack(offset))
    ...
    # update meta for dense layout
    meta["layout"] = "dense"
    meta["denseEncoding"] = {
        "rowHeaderBytes": U16_LE.size,
        "attrIndexBytes": ATTR_INDEX_BYTES,
        "valueIndexBytes": DICT_INDEX_BYTES,
        "offsetBytes": ROW_OFFSET_BYTES,
        "rowsFile": rows_path.name,
        "offsetsFile": offsets_path.name,
    }
    ...
    print(
        f"[dense] {type_name}: done, rows={rows_size / (1024*1024):.2f} MiB, "
        f"offsets={offsets_size / (1024*1024):.2f} MiB"
    )

    # update stats for downstream summary + total size
    stats = dict(stats)
    stats["layout"] = "dense"
    stats["binSize"] = rows_size
    stats["offsetsSize"] = offsets_size
    return stats
...
def fetch_relations_to_tmp_uuid_pairs(
    ...
    Each record: 2 * UUID_BYTES bytes = src_uuid + tgt_uuid
    """
...
def pack_relations_from_uuid_pairs(
    ...
    for rel in sorted(reltypes):
        ...
        record_bytes = 2 * UUID_BYTES
        if file_size % record_bytes != 0:
            print(
                f"[WARN] {pair_path} size not divisible by {record_bytes}; possible corruption",
                file=sys.stderr,
            )
        record_count_raw = file_size // record_bytes

        with pair_path.open("rb") as f:
            for _ in range(record_count_raw):
                src_b = f.read(UUID_BYTES)
                tgt_b = f.read(UUID_BYTES)
                if len(src_b) != UUID_BYTES or len(tgt_b) != UUID_BYTES:
                    break
...
def write_packed_manifest(
    ...
    if nodes_dir.is_dir():
        for meta_path in nodes_dir.glob("*.meta.json"):
            ...
            if layout == "dense":
                entry["binFile"]     = f"{tname}.rows.bin"
                entry["offsetsFile"] = f"{tname}.rows.offsets.bin"
            else:
                # default: grid legacy layout
                entry["binFile"]     = f"{tname}.bin"
                entry["offsetsFile"] = None
...
def fetch_hybrid() -> None:
    ...
    # 3b) Decide per-type whether to convert grid -> dense
    print("\n=== NODES (hybrid): grid vs dense decision ===")
    updated_type_stats: List[Dict[str, Any]] = []

    for st in type_stats:
        tname        = st["type"]
        N            = st["recordCount"]
        C            = st["blockSize"]
        sum_defined  = st.get("sumDefined", 0)
        grid_bytes   = st["binSize"]

        if N == 0 or C == 0:
            updated_type_stats.append(st)
            continue

        dense_bytes = estimate_dense_bytes(N, C, sum_defined)

        print(
            f"[hybrid] {tname}: grid={grid_bytes / (1024*1024):.2f} MiB, "
            f"dense_est={dense_bytes / (1024*1024):.2f} MiB"
        )

        # Simple policy: convert only if dense is strictly smaller.
        # You can tighten this with a margin (e.g. dense_bytes < 0.98 * grid_bytes).
        if dense_bytes < grid_bytes:
            st = convert_node_type_grid_to_dense(tname, st)
        else:
            print(f"[hybrid] {tname}: keeping grid layout")

        updated_type_stats.append(st)

    type_stats = updated_type_stats
    ...
    for st in type_stats:
        ...
        print(
            ...
            f"offsets={st.get('offsetsSize', 0) / (1024*1024):.2f} MiB"
        )
